    ...
    if(operation == 'u'){//u for update
     ...
     if(operation == 'd'){//d for delete
        ...
        public void showAllScores(JTable table)
        {
            ...
            PreparedStatement ps;
            try {
                ps = con.prepareStatement("SELECT `student_id`, first_name, last_name, label, student_score FROM `score` INNER JOIN student as stab on stab.id = student_id INNER JOIN course as ctab on ctab.id = course_id");
                ResultSet rs = ps.executeQuery();
                DefaultTableModel model =(DefaultTableModel) table.getModel();
